...
description: Use when debugging any non-trivial bug — wrong output, crash, flaky test, performance regression, or "it works locally but not in CI." Forces a scientific-method loop (Observe → Hypothesize → Experiment → Conclude) so the agent stops guessing and starts reasoning. Prevents the #1 AI debugging failure mode — bulldozing through a wrong idea instead of falsifying it. 中文觸發詞：『這個 bug 怎麼修』、『為什麼測試會掛』、『一直修不好』、『同一個 bug 又出現』、『科學方法除錯』、『不要亂猜』、『debug 卡住了』、『本地會過 CI 卻掛』等。
---
